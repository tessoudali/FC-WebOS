...
new explorer.window()
.title('FloydCrft MapApp for WebOS') //set the title
.resize(1140, 700) //set the dimentions
.center()
.icon('programs/MapApp/FCMaps.ico') //set the icon
.callback(function() {
	...
	function changeUrl(url) {
		...
		win.title('FloydCraft MapApp for WebOS - '+url)
	};
	
	//Create the starting HTML
	bod.html('<button class="b1">🢀</button><button class="b2">🢂</button><input class="i1"></input><button class="b3">Go</button><iframe src="http://maps.floydcraft.co.uk/MapApp2021/bg.html" class="if1"></iframe>');
	
	//Give the button its position via the style (CSS) and piggyback a click event if it is clicked.
	bod.find('button.b1').attr('style', 'position:absolute;left:0px;top:0px;width:0px;height:0px;').click(function() {
		//If button is clicked find the iframe and go back 1 page in its history.
		bod.find('iframe.if1')[0].contentWindow.history.go(-1);
	});
	
	//Give the button its position via the style (CSS) and piggyback a click event if it is clicked.
	bod.find('button.b2').attr('style', 'position:absolute;left:0px;top:0px;width:0px;height:0px;').click(function() {
	...
	bod.find('button.b3').attr('style', 'position:absolute;right:0px;top:0px;').click(function() {
	...
	bod.find('input.i1').attr('style', 'position:absolute;top:0px;left:0px;width:calc(100% - 100%);').keypress(function(event) {
	...
	bod.find('iframe.if1').attr('style', 'position:absolute;top:0px;left:0px;width:100%;height:calc(100% - 0px);border:none;background-color:white;');
	...
	function startTracker() {
		...
		var interval = setInterval(function() {
			...
			try {
				...
				win.title('FloydCraft MapApp for WebOS');
